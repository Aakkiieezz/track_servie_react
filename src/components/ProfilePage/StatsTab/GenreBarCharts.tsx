...
const GenreBarCharts: React.FC<Props> = ({ userId, onFetchError }) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axiosInstance.get(`stats/${userId}/genre-freq`);
				// Sort by value highest to lowest
				const sortedMovies = response.data.movies.sort((a: GenreData, b: GenreData) => b.value - a.value);
				const sortedTVShows = response.data.tvShows.sort((a: GenreData, b: GenreData) => b.value - a.value);
				setDataMovies(sortedMovies);
				setDataTVShows(sortedTVShows);
			} catch (error) {
				onFetchError("Failed to fetch genre freq stats");
			} finally {
				setLoading(false);
			}
		};
	...
	const renderCustomLabel = (props: any) => {
		...
		if (width > LABEL_WIDTH + BAR_PADDING) return (
			<text
